...
export function liveUpdate() {
  ...
  onValue(sortedMessages, (snapshot) => {
    ...
    if (messages) {
      ...
      messageContainer.innerHTML =
        '<p class="no-messages">Low on good vibes, be the first to send some!</p>';
    }
  });
}
...
export async function addMsg(text, gifUrl, color, nickName) {
  const resultData = {
    text: text,
...
function render(text, id, createdAt, likes, gifUrl, color, nickName) {
  ...
  deleteBtn.addEventListener("click", async () => {
    const confirmDelete = confirm(
      "Are you sure you want to remove the good vibes? This cannot be undone.",
    );
...
form.addEventListener("submit", async (e) => {
  ...
  const nickName =
    document.querySelector("#nickName").value.trim() || "Anonymous";
  const gifUrl = getSelectedGif();
  if (!text) return;

  const censoredText = censorBadWords(text);

  await addMsg(censoredText, gifUrl, selectedColor, nickName);
